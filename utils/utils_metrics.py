...
def compute_mIoU(gt_dir, pred_dir, png_name_list, num_classes, name_classes):
    ...
    # pred_imgs = [join(pred_dir, x) for x in os.listdir(pred_dir)]
    # gt_imgs = [join(gt_dir, x + ".tif") for x in png_name_list]
    gt_imgs = [join(gt_dir, x.replace("_", "_gt_") + ".bmp") for x in png_name_list]
    ...
    dice_final = np.nanmean(np.array(dice_total))
    # ------------------------------------------------#
    #   逐类别输出一下mIoU值
    # ------------------------------------------------#
    ...
    print("dice score: {}".format(dice_final))
